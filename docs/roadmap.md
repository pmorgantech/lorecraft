...
- [ ] 72.3a — **Admin "request restart" endpoint (safe half, shippable now).** Admin-auth'd endpoint + button
  that records a restart request (touch a sentinel file or send a signal), audit-logged, with a confirmation
  gate and an "armed?" indicator that reflects whether a performer is actually listening. No process-lifecycle
  code in the request path. *Success: clicking restart (with confirm) writes the sentinel / sends the signal and
  audit-logs it; with no performer wired it shows "not armed," never a silent no-op; lives entirely in
  `webui/admin/` (no engine touch, no tier violation).*
- [ ] 72.3b — **Performer — BLOCKED on the product/ops fork above (A supervisor vs. B exec-replace).** Do not
  start until the user picks the model. *If A:* split `start.sh` into cold-boot prep vs. a reseed-free relaunch
  loop; add the SIGTERM→wait→relaunch performer + restart-storm guard; verify players re-attach via the grace
  `reconnect_sync` cushion (DB preserved). *If B:* add the guarded in-process `os.execv` self-replace; document
  the abrupt-drop / no-lifespan-shutdown / cold-reconnect limitations. *Success (A): a requested restart brings
  the process back with players seamlessly reconnected and runtime state intact; a crash also auto-recovers.
  Success (B): a requested restart reloads code in-place, port rebinds sub-second, players reconnect (cold) with
  runtime state intact.*
- [ ] 72.3c — **Regression guard for the reseed footgun.** Whichever performer lands, add a test/assertion that
  a *restart* (not a cold boot) does **not** invoke `reset_runtime_db` / reseed — i.e. live runtime state
  survives a restart. *Success: a restart preserves a mutated runtime DB; a cold boot still reseeds.*
