...
| 58.6 | **Ledger layout (1b).** Left column = Location + Map; Chronicle runs wide in the centre; secondary panels (Here / Quests / Pack / Stats) collapse into a slim right vertical rail using the 58.5 mechanism. Pure CSS-grid re-flow + rail — no data changes. | [~] **First pass:** stable `.game-col-left/-center/-right` hooks in `game.html` + CSS proportion reflow (narrow flanks, wide centred chronicle with a 72ch reading measure), scoped to `≥1024px`. Cross-column panel moves (inventory → right rail) deferred pending visual review — kept to CSS-only so it can't break the flex grid. |
