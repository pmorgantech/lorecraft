...
The three UI-cohesion backlog items added the same day (2026-07-13, commit `728bc8b`) are also
fully closed: info-command output consolidation (`score`/`abilities`/`train`/`traits`/`skills`/
`reputation` — commit `2343da8`), `help`'s CSS styling investigation (resolved as
working-as-designed; `docs/issues.yaml`'s `issue-fd64ee3e` closed — commit `95b5165`), and HP
added to the vitals line plus its broadcast across all five layout templates (commit `446c823`)
— see the struck-through Backlog table entries below for the resolution detail on each.
