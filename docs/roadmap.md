...
**Status: complete (v0.41.0 → v0.41.4).** Harness (H1–H3) + **12 new e2e tests** shipped: P1 (5,
multiplayer/WS), P2 (5, auth), P3 (3, interaction), P4 (2, panels). Three subtasks were **deferred
with concrete findings** rather than forced to pass — each because the Ashmoore world or the harness
lacked the assumed capability, and fabricating content/hooks solely to pass a test would violate the
repo's data-driven / no-reward-hacking rules:
- **P3.3** (locked door → key): Ashmoore has no locked *exits* (`cage_lock`/`cage_key` are items).
- **P4.2** (equipment/wield): Ashmoore ships **no equippable items** (no item sets `Item.slot`) — a
  real content gap worth filling later.
- **P5.1** (reconnect/resync): Playwright `set_offline(True)` doesn't sever an open WebSocket in this
  Chromium (verified via `window.Lorecraft.isConnected()`), so the disconnect can't be simulated
  without a force-close hook or server bounce.
...
| 50.3 | **Harness H3: offline toggle** (only for P5.1 reconnect test). Playwright `context.set_offline(True/False)` to exercise `app.js` reconnect + `reconnect_sync` backfill. Kept separate because it is timing-sensitive. | [x] `set_offline(page, offline)` wraps Playwright's `context.set_offline()`. **Caveat discovered during P5:** `set_offline(True)` does **not** sever an already-open WebSocket in this Chromium (`window.Lorecraft.isConnected()` verified to stay `true` for the whole offline window) — so it can't actually simulate a WS disconnect. See P5 (50.8). (v0.41.0) |
...
| 50.6 | **Priority 3 — Interaction flows (extend `test_gameplay_flows.py`):** P3.1 command history ArrowUp/ArrowDown multi-entry + index reset; P3.2 full dialogue traversal + dismiss; P3.3 locked door → key golden path (multi-step regression anchor); P3.4 invalid command robustness. | [x] P3.1/P3.2/P3.4 passing (v0.41.2). **P3.3 deferred:** Ashmoore has no locked *exits* — `cage_lock`/`cage_key` are items in the locksmith disambiguation set, not an exit lock, so the "locked door → key" content doesn't exist; the exit lock/unlock/move mechanic is unit/integration-tested, and adding shipped world content just to pass one e2e would break the data-driven/no-reward-hacking rule + the audit golden. Reopen if a locked exit is added to Ashmoore as real content. |
| 50.7 | **Priority 4 — Panel rendering (`test_panel_rendering.py`):** P4.1 minimap current-room highlight moves on movement; P4.2 equipment/wield/wear/unwield flow; P4.3 feed autoscroll + top/bottom controls. | [x] P4.1/P4.3 passing (v0.41.3). **P4.2 deferred:** Ashmoore ships no equippable items (no item sets `Item.slot`, so `item_fits_slot` rejects every wield/wear and the inventory panel shows no equipped state); the equip mechanic is unit/integration-tested, and adding a wieldable item just to pass one e2e would break the data-driven/no-reward-hacking rule + audit golden. A real content gap worth filling later. |
| 50.8 | **Priority 5 — High-value but flaky (P5.1 reconnect test).** WS reconnect / resync backfill: A and B connected; set B offline; A acts (missed); set B online; `app.js` reconnect + `reconnect_sync` / `feed?since=` should backfill. Assert (with generous polling) B's feed eventually contains the missed line. Implement last with long `wait_for_function` timeouts. | **Deferred (v0.41.4)** — not e2e-testable with the available harness. The proposed mechanism (H3 `context.set_offline`) does **not** disconnect an already-open WebSocket in this Chromium: probed `window.Lorecraft.isConnected()` and it stays `true` for the entire offline window, and a "missed" message is delivered live over the still-open socket (a false positive). A real reconnect/resync e2e needs a way to force-close B's socket (a test-only `ws.close()` hook) or to bounce the server — a production change/harness build-out beyond a test-only sprint, and the reconnect grace-period + `reconnect_sync` payload are already covered in the integration/simulation tier. Reopen with a force-close hook if this path regresses. |
