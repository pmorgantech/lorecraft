...
| 58.6 | **Ledger layout (1b).** Left column = Location + Map; Chronicle runs wide in the centre; secondary panels (Here / Quests / Pack / Stats) collapse into a slim right vertical rail using the 58.5 mechanism. Pure CSS-grid re-flow + rail — no data changes. | [x] Narrow left (Location + Map only) + **wide full-width chronicle** (the 72ch cap that starved it was removed after review) + slim right rail. Inventory moves out of the left column into a right-rail **Inventory/Quests window-shade accordion** (Alpine, one open at a time; both stay in the DOM so `#inventory`/`#quest-tracker` OOB updates still fire). `test_ledger_layout_moves_inventory_to_right_rail` asserts the relocation + single render. |
| 58.7 | **Dock layout (1c).** A visible control bar (theme · density · layout · panel toggles surfaced from `/settings` inline) above card-style panels, plus the rarity-coloured **icon-grid inventory** variant. Drag-to-reorder panels is a **stretch** (behind a flag) — the reviewable core is the toolbar + card treatment + icon-grid. | [~] **First cut:** CSS **card treatment** — panels float as spaced, rounded, shadowed cards over the app background, visibly distinct from standard's flush grid. Control toolbar + rarity icon-grid inventory still to come. |
| 58.8 | **Immersive layout (1d) + docs.** Near-full-bleed Chronicle with a soft vignette; everything else collapses to a slim icon rail (58.5) that expands on demand; floating minimap + floating command bar. Document both axes in [`user_guide.md`](user_guide.md); changelog; render tests asserting the `layout-*` body class and that hidden-by-default rail panels are still reachable. | [~] **First cut:** CSS **low-chrome cinematic** — slim flanks, a dominant larger-type chronicle, near-borderless panels, and a soft inner vignette. Slim icon rail + floating command/minimap still to come. |
| 58.9 | **Live theme/layout preview.** The Settings **Theme**/**Layout** dropdowns preview immediately (Alpine swaps the `theme-*`/`layout-*` body classes on change); **Save** persists via the existing POST, **Cancel** returns to `/game` and reloads the last-saved prefs (natural revert). | [x] `settings.html` Alpine `applyPreview()`; hint text on the button row. |
