...
| 56.5 | Sweep existing `ctx.say(...)` call sites in `engine/` and `features/`; assign a type where the intent is clear from context, leave genuinely ambiguous ones on the `"system"` default rather than guessing. | [~] `quests/service.py` + `hunts/service.py` + `marks/service.py` → `QUEST`; `commands/social.py` + `npc/dialogue.py`'s precondition failures → `WARNING`; `npc/dialogue.py`'s actual NPC speech line → `TELL`; `inventory/service.py`'s ~59 precondition/disambiguation/exception-passthrough messages → `WARNING`, its ~27 success-narration/look-output messages left on `SYSTEM` (not warnings, not a clean fit for any other type). Sampled `fatigue/service.py` and found no clean fit there either — left on `SYSTEM` rather than force a stretch mapping. ~200 call sites across the remaining 21 files are still on the `SYSTEM` default (biggest: `economy/service.py` 20, `trading/service.py` 18, `transit/service.py` 16); a full sweep is a follow-on, not blocking. |
