...
| Latent fire-and-forget task tracking in `broadcast_room_async` | Added 2026-07-20. `engine/game/world_context.py`'s `broadcast_room_async` schedules room narration via `asyncio.create_task()` without keeping the task reference — safe today because every caller either has no synchronous "drain" step right after it (autonomous NPC/weather ticks) or goes through the real `ConnectionManager`, whose actual socket I/O happens to yield the event loop before anything reads results back out. A branch doing in-process command replay against a synchronous, zero-I/O connection-manager stand-in surfaced this as a real, reproducible dropped-narration bug: a trigger-fired `narrate_room` call raced a synchronous drain immediately following it, with nothing forcing the scheduled task to run first. No such caller exists in this codebase today, so no fix is needed here yet — flagging the latent gap so a future synchronous or test-double `ConnectionManager` consumer (e.g. a replay/simulation harness) doesn't reintroduce it silently. If one is ever added, the fix is to have `broadcast_room_async` accept an optional task sink (e.g. a `pending_narration_tasks` list) that callers can `await`/`gather` before their drain step — not a `sleep(0)`/yield-count hack, which is fragile and implementation-coupled. |
