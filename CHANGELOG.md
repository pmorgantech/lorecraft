...
- **Social emotes: `wave` and `point`.** `wave [at <target>]` and `point at <target>` broadcast to
  the room; a target resolves to a co-located NPC or player by name, otherwise the raw text (so
  `point at sign` / `wave at the sky` just work).
- **Player `quests` command** (alias `quest`). Lists your quests with status; a multi-stage quest
  shows `stage N/M` and the current stage's objective, and completed/failed quests are marked.
  Read-only — quest progression stays event-driven. Equipment already has full coverage
  (`wear`/`wield` equip, `remove`/`unwield` unequip); no `equip`/`unequip` aliases were added.
...
- **World roadmap Phase 1 complete: Whisperwood + Port Veridian zones.** 28-room
  Whisperwood ancient forest (`area_id: whisperwood` — forest floor, canopy city,
  two-level cave system) with Ranger Elena and Geomancer Shard, and 22-room Port
  Veridian coastal settlement (`area_id: port_veridian` — docks, shipyard,
  fisherfolk quarter, warehouse district with a locked/keyed vault, lighthouse)
  with Captain Iris, Tavern Keeper Sal, Lighthouse Keeper, and Shipwright
  Calloway. World totals: 96 rooms, 78 items, 10 NPCs, 3 quests. Completes all
  three Phase 1 zones from `docs/roadmap_world.md` (Cogsworth/Whisperwood/Port
  Veridian foundation skeletons).
